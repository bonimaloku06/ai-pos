...
export async function syncProductToSearch(product: any) {
  try {
    ...
    console.error("Failed to sync product to search:", error);
  }
}
...
// Search products
export async function searchProducts(query: string, options: { limit?: number; offset?: number; filter?: string } = {}) {
  try {
    const index = meiliClient.index("products");
